...
def test_score_api_round_trip(client) -> None:
    ...
    scores = client.get("/api/scores?game=pong").get_json()
...
def test_metrics_api_round_trip_and_export(client, tmp_path) -> None:
    ...
    db_path = Path(client.application.config["SERVICES_DB"])
...
def test_metrics_missing_run_is_rejected(client) -> None:
    response = client.post(
        "/api/metrics/batch",
