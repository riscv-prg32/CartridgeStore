"""Export one PRG32 metrics run from the consolidated service database."""
...
from .metrics_report import generate_markdown_report, load_run, load_samples, summarize_samples


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_DB = Path(os.environ.get("PRG32_STORE_DB", ROOT / "data" / "services.sqlite3"))
...
def export_run(db_path: Path, run_id: str, output_dir: Path) -> None:
    ...
    (output_dir / "report.md").write_text(
        generate_markdown_report(db_path, run_id),
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    parser.add_argument("--db", default=str(DEFAULT_DB), help="SQLite service database path")
    parser.add_argument("--out", default="metrics_export", help="export directory")
